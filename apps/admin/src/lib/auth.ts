...
/**
 * Browser auth helpers for the admin console. Requests use `credentials:
 * 'include'` so the API's httpOnly refresh cookie is set and sent.
 */
...
/**
 * Marker cookie so middleware can keep signed-out admins out of the app shell.
 * The real session cookie is httpOnly and set by the API on its own domain, so
 * in a split deployment (API and console on different hosts) middleware running
 * here can never see it. This first-party marker carries no authority — the API
 * still authorizes every call — it only drives routing.
 */
